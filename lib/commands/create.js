  ...
  program
    ...
    .action(async function (shortName, options) {
      ...
      if (options.mongodbUri) {
        ...
        createUserCommand = `APOS_MONGODB_URI="${options.mongodbUri}" ` + createUserCommand;
        util.log('create', 'Configuring MongoDB URI in .env file');
        // Write or append to the .env file
        if (!fs.existsSync('.env')) {
          fs.writeFileSync('.env', '');
        }
        fs.appendFileSync('.env', `APOS_MONGODB_URI="${options.mongodbUri}\n"`);
      }
