...
module.exports = function (program) {
  program
    ...
    .action(async function(shortName, options) {
      ...
      // Format the URL according to your rules
      const boilerplateUrl = /^(http|https):\/\//i.test(input)
        ? input
        : input.includes('/')
          ? `https://github.com/${input.startsWith('/') ? input.slice(1) : input}`
          : `https://github.com/apostrophecms/starter-kit-${input}.git`;

      util.log('create', `Grabbing the starter from Github [1/4] https://github.com/apostrophecms/starter-kit-${input}.git`);
