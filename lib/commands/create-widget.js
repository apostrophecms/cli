...
module.exports = function (program, version) {
  program
    ...
    .action(function(moduleName, options) {
      ...
      fs.writeFileSync(path.join(modulePath, 'views/widget.html'), '');
      ...
      if (options.player) {
        util.log('create-widget', `Setting up lean.js for ${fullWidgetName}.`);
        mkdir('-p', path.join(modulePath, 'public/js'));

        const jsConfig = strings.jsConfig;

        fs.writeFileSync(path.join(modulePath, 'public/js/lean.js'), jsConfig);
      }
