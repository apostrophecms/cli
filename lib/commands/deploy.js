...
async function addServer() {
  ...
  const keyDir = `${os.homedir()}/.ssh`;
  if (!fs.existsSync(keyDir)) {
    fs.mkdirSync(keyDir);
    fs.chmodSync(keyDir, 0o700);
  }
  const keyFile = `${keyDir}/aposapps`;
  if (!fs.existsSync(keyFile)) {
    exec(`ssh-keygen -b 2048 -t rsa -f ${keyFile} -q -N ""`);
  ...
  await sshRoot(server, stripIndent`
    ...
    if id -u aposapps > /dev/null 2>&1; then
      ...
      useradd -m -s /bin/bash aposapps || exit 1
    fi
    if [ -f /home/aposapps/.ssh/aposapps.pub ]; then
      echo "The deployment ssh key already exists"
    else
      echo "installing the aposapps public key"
      cat > /home/aposapps/.ssh/aposapps.pub <<EOF
${key}
      EOF
  `);
